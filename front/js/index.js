...
function createPost(feedPosts) {
    ...
    feedPosts.forEach((value, ind) => {
        ...
        let time = value.date
        postAuthorSectionPostDate.textContent = time; //posto autoriaus post date
        ...
        //like mygtukas
        if (value.likesList.length > 0) {
            postActionButtonsLike.innerHTML = '<ion-icon name="heart-empty" style="fill: red;"></ion-icon>'
            console.log(1)
        } else {
            postActionButtonsLike.innerHTML = '<ion-icon name="heart-empty"></ion-icon>'
            console.log(2)
        }
